...
export default function Edit(props) {
    ...
    const { data, setData, post, errors, processing } = useForm({
        ...
        kategori_id: props.barang.kategori_id || null,
        harga_dasar: props.barang.harga_dasar || '',
        harga_jual: props.barang.harga_jual || '',
        supplier_id: props.barang.supplier_id || null,
        unit: props.barang.unit || null,
        diskon: props.barang.diskon || '',
        foto: props.barang.foto || '',
    })

    useEffect(() => {
        // Menetapkan nilai awal untuk selectedKategori
        if (props.kategori && props.barang.kategori_id) {
            const initialKategori = props.kategori.find(kategori => kategori.id === props.barang.kategori_id);
            setSelectedKategori(initialKategori);
        }

        // Menetapkan nilai awal untuk selectedSupplier
        if (props.supplier && props.barang.supplier_id) {
            const initialSupplier = props.supplier.find(supplier => supplier.id === props.barang.supplier_id);
            setSelectedSupplier(initialSupplier);
        }
    }, [props.kategori, props.supplier, props.barang.kategori_id, props.barang.supplier_id]);
    ...
    const submit = (e) => {
        e.preventDefault()
        post(`/edit-barang/${props.barang.id}`, {
            preserveScroll: true,
            onSuccess: () => {
                    ...
                    <div className="bg-white shadow-lg rounded-lg p-2 md:p-6 border border-gray-200 mb-16">
                        <div className="overflow-x-auto p-2">
                            <form onSubmit={submit}>
                                <div className="mb-4">
                                    <label for="nama_barang" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Kategori</label>
                                ...
                                <div className="mb-4">
                                    ...
                                    <select id="unit" value={data.unit} onChange={(e) => setData('unit', e.target.value)} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
