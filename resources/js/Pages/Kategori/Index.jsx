...
export default function Index(props) {
    ...
    const [filteredData, setFilteredData] = useState((props.kategori || []).map((item, index) => ({
    ...
    const handleSearch = (event) => {
        ...
        const filtered = (props.kategori || []).map((item, index) => ({
            no: index + 1,
            ...item,
        })).filter(item =>
            item.nama_kategori.toLowerCase().includes(searchValue) ||
    ...
    const ExpandedComponent = ({ data }) => (
        <div className="p-4">
            <div className="flex justify-start">
                <Link replace href={route('kategori.edit', data.id)}>
                ...
                <main className="flex-1 p-4 md:p-6 bg-white border-l border-gray-300 mt-16 overflow-auto mb-16">
                    ...
                    <div className="bg-white shadow-lg rounded-lg p-4 md:p-6 border border-gray-200 relative">
                        <div className="flex flex-col md:flex-row justify-between mb-4 space-y-4 md:space-y-0 md:space-x-4">
                            <Link replace href={route('kategori.create')}>
